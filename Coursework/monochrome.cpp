...
monochrome::monochrome(QDialog* parent): QDialog(parent)
{
    setWindowTitle("Black/White");
    ok = new QPushButton;
    ok->setText("OK");
    label = new QLabel;
    ...
    connect(ok, SIGNAL(clicked()), this, SLOT(do_monochrome()));
}
...
void monochrome::do_monochrome() {
    emit change_monochrome(coord_1x->value(), coord_1y->value(), coord_2x->value(), coord_2y->value());
}
